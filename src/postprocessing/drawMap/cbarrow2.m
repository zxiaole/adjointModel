...
if makeuparrow
    ...
    hu = patch(uparrowx,uparrowy,cm(end,:),'EdgeColor',cm(end-1,:)); 
end

if makedownarrow
    downarrowx = cbpos(1) + [0 cbpos(3) cbpos(3)/2 0]; 
    downarrowy = cbpos(2) + [0 0 -cbpos(3)*sqrt(2)/2 0]; 
    hd = patch(downarrowx,downarrowy,cm(2,:),'EdgeColor',cm(2,:)); 
end

%% Change edge colors: 

if strcmpi(get(cb, 'box'),'on')
    
    % Get starting color and linewidth of colorbar box: 
    linecolor = [0 0 0];% get(cb, 'color'); 
    linewidth = get(cb, 'LineWidth'); 
...
%     cb.Box = 'off';
    ...
    if all([makeuparrow makedownarrow])
        line(cbpos(1) +[0 0 cbpos(3)/2 cbpos(3)],...
            cbpos(2) + [0 cbpos(4) cbpos(4)+cbpos(3)*sqrt(2)/2 cbpos(4)],...
            'color',linecolor,'LineWidth',linewidth)
        line(cbpos(1) +[cbpos(3) cbpos(3)/2 0],...
            cbpos(2) + [ 0 -cbpos(3)*sqrt(2)/2 0],...
            'color',linecolor,'LineWidth',linewidth)
    end
