...
int main(int argc, char** argv)
{
  ...
  if (exists(sec_config_file))cout << "part1";
  ...
  Data<real, 1> particleDiameter(GridS);
  Data<real, 1> particleDensity(GridS);
  ...
  InputParam.Read(SpeciesData, particleDiameter);
  InputParam.Read(SpeciesData, particleDensity);
  ...
  real stab, rich, Cd(0.), Ra, Rb, Rd, Rc(0.);
                ...
                for (l = 0; l < Nc; l++)
                  {
                    ...
                    real mu = 0.000001458*pow(SurfaceTemperature(h, j, i) + 273.16, 1.5)/(SurfaceTemperature(h, j, i) + 273.16 + 110.4);

                    real rho = (10 + 273.16)/(SurfaceTemperature(h, j, i) + 273.16)*1.246;

                    real nu = mu/rho;


                    // Scf Cunningham slip correction factor
                    real lamda = 0.000000065;//pow(10.0, -9.0);
                    real dp = particleDiameter(s);
                    real rhoParticle  = particleDensity(s);

                    real Kn = 2*lamda/dp;

                    real alpha = 1.257;
                    real beta = 0.4;
                    real Cc = 1 + Kn*(alpha + beta*exp(-1.0*lamda/Kn));

                    real k = 0.0000000000000000000000138;//pow(10.0, -23.0);

                    real D = k*(SurfaceTemperature(h, j, i) + 273.16)/3.0/pi/mu/dp*Cc;

                    // Schmidt number
                    real Sc = nu/D;

                    // friction velocity
                    real Ustar = FrictionVelocity(h, j, i);

                    // settling velocity
                    real Vg = (rhoParticle - rho)*g*dp*dp*Cc/18.0/mu;

                    // Stokes number
                    real St = (Vg/g)*Ustar*Ustar/nu;

                    real Rd = (1.0/Ustar)/(pow(Sc, -2.0/3.0) + pow(10.0, -3.0/St))/1.0;
                    ...
                    //Velocities(h, j, i) += LandUse(l, j, i)*(Vg + 1/(Ra + Rd)) ;
                    Velocities(h, j, i) += LandUse(l, j, i)*(Vg/(1-exp(-(Ra+Rd)*Vg)));
...
cout << "Done for " << dir_out + SpeciesNames(s) + ".bin" << endl;
      // Writes output files.
